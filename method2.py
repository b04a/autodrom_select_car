...
async def main():
    async with async_playwright() as p:
        ...
        page_count = 1
        await page.goto(f'https://auto.drom.ru/all/page{page_count}/?minprice=100000&maxprice=200000', wait_until='domcontentloaded')
        ...
        while True:  # Бесконечный цикл для перехода по страницам
            while True:  # Цикл для автомобилей на текущей странице
                ...
                while i < car_count:  # Вложенный цикл по автомобилям
                    ...
                    car_name = await car_name_locator.text_content() if await car_name_locator.count() > 0 else "Неизвестно"
                    car_price = await car_price_locator.text_content() if await car_price_locator.count() > 0 else "Неизвестно"
                    ...
                    if i == 19:
                        print(f"Обработано {i} автомобилей. Переход на следующую страницу.")
                        break  # Выход из цикла автомобилей

                if i == 19:
                    page_count += 1
                    await page.goto(f'https://auto.drom.ru/all/page{page_count}/?minprice=100000&maxprice=200000',
